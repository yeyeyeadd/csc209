...
int main(int argc, char *argv[]) {
  ...
  Dataset *traningdata;
  Dataset *testingdata;
  traningdata = load_dataset(argv[1]);
  testingdata = load_dataset(argv[2]);
  DTNode *node;
  node = build_dec_tree(traningdata);
  int label;
  for (int i = 0; i < testingdata->num_items; i++){
      label = dec_tree_classify(node, &(testingdata->images[i]));
      if (label == testingdata->labels[i]){
        total_correct += 1;
      }
  }
  // Print out answer

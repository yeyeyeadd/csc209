...
int main(int argc, char *argv[]) {
    ...
    // TODO
    double (*ptr) (Image *, Image *);

    if (strncmp(dist_metric, "euclidean", strlen(dist_metric)) == 0 || 
    strncmp(dist_metric, "eucl", strlen(dist_metric)) == 0){
        ptr = distance_euclidean;
    }
    else if (strncmp(dist_metric, "cosine", strlen(dist_metric)) == 0 ||
    strncmp(dist_metric, "cos", strlen(dist_metric)) == 0) {
        ptr = distance_cosine;
    }
    else {
        printf("wrong distance function");
        exit(1);
    }
    ...
    for (int i = 0; i < num_procs; i++){
        ...
        if (n < 0){
            ...
            close(fd[i][1]);
            close(fd2[i][0]);
            child_handler(training, testing, K, ptr, fd[i][0], fd2[i][1]);
            close(fd[i][0]);
            close(fd2[i][1]);
            exit(0);
        }
            ...
            close(fd[i][0]);
            close(fd2[i][1]);
            if (write(fd[i][1], &index, sizeof(int)) == -1){
                perror("write");
                exit(1);
            }
            if (i == num_procs - 1){
                int b = testing->num_items - (N * i);
                if (write(fd[i][1], &b, sizeof(int)) == -1){
                    perror("write");
                    exit(1);
                }
            }    
            else {
                if (write(fd[i][1], &N, sizeof(int)) == -1){
                    perror("write");
                    exit(1);
                }
            }    
    ...
    for (int i = 0; i < num_procs; i++){
        if (wait(NULL) == -1) {
            printf("There is a error in  child process.");
        }
    }
    ...
    for (int i = 0; i < num_procs; i++){
        from_child = 0;
        if (read(fd2[i][0], &from_child, sizeof(int)) == -1){
            perror("read");
            exit(1);
        }
        close(fd2[i][0]);
        total_correct += from_child;
    }

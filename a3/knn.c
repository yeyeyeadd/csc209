...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    for (int i = start_idx; i < start_idx + N; i++){
        p = knn_predict(training, &testing->images[i], K, fptr);
        if (p == testing->labels[i]){
            acc ++;
        }
    }
    if (write(p_out, &acc, sizeof(int)) == -1) {
        perror("write");
        exit(1);
    }
...
double distance_cosine(Image *a, Image *b){
    ...
    for (int i = 0; i < a->sx * a->sy; i++) {
        ...
        denominatora += a->data[i] * a->data[i];
        denominatorb += b->data[i] * b->data[i];
    }
    distance = 2*acos(numerator / ((sqrt(denominatora) * sqrt(denominatorb)))) / M_PI;
    return distance;
}
